...
export default function ScrollStory({ onOpenProjectModal }: ScrollStoryProps) {
  ...
  // Subtle Hero intro text overlay that lives ONLY in negative space (sides)
  ...
  return (
    ...
    >
      {/* Sticky Viewport (Pinned at top: 0 for 100vh until 400vh ends) */}
      <div className="sticky top-0 left-0 w-full h-screen overflow-hidden flex items-center justify-center bg-[#050505]">
        ...
        {/* Minimal Hero Perimeter Content (Negative Space Only - Center Remains 100% Clean) */}
        <motion.div
          style={{ opacity: heroTextOpacity, y: heroTextY }}
          className="absolute inset-0 z-10 pointer-events-none p-6 sm:p-10 md:p-14 flex flex-col justify-between"
        >
          {/* Top Row: Negative space header tags (below navbar) */}
          <div className="pt-20 md:pt-24 flex items-center justify-between pointer-events-auto">
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-white/10 bg-[#111114]/70 backdrop-blur-md">
          ...
          {/* Bottom Row: Left description & Right CTAs */}
          <div className="pb-6 flex flex-col md:flex-row items-end justify-between gap-6 pointer-events-auto">
            {/* Bottom-Left: Subdued Manifesto Tag */}
            <div className="max-w-xs sm:max-w-sm text-left">
              <div className="text-xs font-mono uppercase tracking-widest text-[#B7FF3C] mb-1.5">
            ...
            <div className="hidden md:flex flex-col items-center gap-1.5 text-white/40 pb-2">
