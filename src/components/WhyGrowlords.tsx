...
export default function WhyGrowlords() {
  return (
    <section id="why" className="relative w-full py-32 bg-[#050505] text-white border-t border-white/5 overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
          {pillars.map((pillar, idx) => (
            <motion.div
              ...
              className="group relative p-8 sm:p-12 rounded-3xl border border-white/10 bg-[#0A0A0C] hover:border-[#B7FF3C]/40 hover:bg-[#111114] transition-all duration-500"
            >
              {/* Corner Pillar Number */}
              <div className="flex items-center justify-between mb-8 pb-6 border-b border-white/10">
                <span className="text-3xl sm:text-4xl font-extrabold font-heading text-white/20 group-hover:text-[#B7FF3C] transition-colors">
                  {pillar.number}
                </span>
                <span className="text-[11px] font-mono tracking-widest uppercase text-white/40 group-hover:text-white/80 transition-colors">
                  PRINCIPLE // {pillar.number}
                </span>
              </div>

              {/* Title */}
              <h3 className="text-2xl sm:text-3xl font-extrabold uppercase tracking-tight font-heading text-white group-hover:text-[#B7FF3C] transition-colors">
                {pillar.title}
              </h3>

              {/* Tagline */}
              <div className="mt-3 text-sm sm:text-base font-semibold text-white/90">
                {pillar.tagline}
              </div>

              {/* Description */}
              <p className="mt-4 text-sm sm:text-base text-white/65 font-light leading-relaxed">
                {pillar.description}
              </p>
            </motion.div>
          ))}
